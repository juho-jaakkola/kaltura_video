...
<?php
	...
	$result = elgg_get_entities(array('types' => 'object', 'subtypes' => 'kaltura_video', 'container_guid' => elgg_get_page_owner_guid(), 'limit' => $number, 'full_view' => FALSE));
	//big thumbnail:
if($result) {
	$body = '';
	...
	$body .= '<a class="tit" href="'.$CONFIG->wwwroot.'pg/kaltura_video/' . elgg_get_page_owner_entity()->username.'">'.elgg_echo("kalturavideo:label:morevideos").'</a>';
	$body .= "</div>\n";
}
else {
	$context = elgg_get_context();
	elgg_set_context("search");
	$body = elgg_list_entities(array('types' => 'object', 'subtypes' => 'kaltura_video', 'container_guid' => elgg_get_page_owner_guid(), 'limit' => $number, 'full_view' => FALSE));
